...
original.p.tree <- read.nexus("parasite.nexus")
...
#### DROP UNWANTED TIPS
trimmed.p.tree <- drop.tip(original.p.tree, c('outgroup1', 'outgroup2', 'duplicate1', etc.))
...
new_tiplabels.p.tree <- c("new_p1", "new_p2", "new_p3", etc.)
...
##### 2) HOST TREE #####
# this is a nexus format. Otherwise, use "read.tree"
original.h.tree <- read.nexus("host.nexus")
...
#### DROP UNWANTED TIPS
trimmed.h.tree <- drop.tip(original.h.tree, c('outgroup1', 'outgroup2', 'duplicate1', etc.))
...
# relabel them in correct order, if desired
new_tiplabels.trimmed.h.tree <- c("new_h1", "new_h2", "new_h3", etc.)
...
pdf("tanglegram.pdf", height = 30, width = 45)
